#!/usr/bin/env python3
"""
Production-Ready Maze Captcha with Full Monitoring and Logging
"""
...
from monitoring import (
    setup_monitoring, maze_logger, performance_monitor,
    monitor_captcha_generation, monitor_verification, monitor_database_operation
)
...
def analyze_and_learn(mouse_data, solve_time, path, session_id=None):
    ...
    if velocity_variance > 50:
        ...
        confidence = 0.2
        maze_logger.log_captcha_event('high_velocity_variance', {
    ...
    if solve_time < 1.0:
        ...
        confidence = 0.3
        maze_logger.log_captcha_event('fast_solve', {
    ...
    if direction_changes < len(path) * 0.1:
        ...
        confidence = 0.4
        maze_logger.log_captcha_event('few_direction_changes', {
...
@app.after_request
    def after_request(response):
        ...
        maze_logger.log_captcha_event('human_pattern_learned', {
            'pattern_count': len(analytics['human_patterns']),
...
def generate_captcha():
    ...
    try:
        ...
        maze_logger.log_captcha_event('captcha_generated', {
            'difficulty': difficulty,
            'captcha_id': captcha_id
        }, getattr(g, 'client_ip', None))
        ...
        maze_logger.log_error(e, {'endpoint': 'generate_captcha', 'difficulty': difficulty})
...
def track_mouse():
    ...
    try:
        ...
        maze_logger.log_error(e, {'endpoint': 'track_mouse', 'captcha_id': captcha_id})
...
def bot_simulate():
    ...
    try:
        ...
        # More realistic bot solve times
        base_solve_time = random.uniform(12, 25)
        solve_time = base_solve_time
        
        if bot_mode == 'perfect':
            solve_time = random.uniform(6, 12)
        elif bot_mode == 'suspicious':
            solve_time = random.uniform(1, 3)
        # Good bot uses normal human-like times
        elif bot_mode == 'good':
            solve_time = base_solve_time
        ...
        if bot_mode == 'perfect':
            # Perfect bot is still fast, but not impossibly fast (6-12 seconds)
            solve_time = random.uniform(6, 12)
        elif bot_mode == 'suspicious':
            # Suspicious bot is unnaturally fast (1-3 seconds)
            solve_time = random.uniform(1, 3)
        # Good bot uses normal human-like times
        elif bot_mode == 'good':
            solve_time = base_solve_time
        ...
        maze_logger.log_captcha_event('bot_simulation', {
            'difficulty': difficulty,
            'mode': bot_mode,
            'detected_as_human': analysis['is_human']
        }, getattr(g, 'client_ip', None))
        ...
        maze_logger.log_error(e, {'endpoint': 'bot_simulate', 'difficulty': difficulty, 'mode': bot_mode})
        return jsonify({'error': 'Bot simulation failed'}), 500
...
def verify_solution():
    ...
    try:
        ...
        if analysis['is_human']:
            ...
            maze_logger.log_captcha_event('human_verified', {
                'captcha_id': captcha_id,
                'confidence': analysis['confidence'],
                'solve_time': solve_time
            }, getattr(g, 'client_ip', None))
            ...
            maze_logger.log_captcha_event('bot_detected', {
                'captcha_id': captcha_id,
        ...
        maze_logger.log_error(e, {'endpoint': 'verify_solution', 'captcha_id': captcha_id})
        return jsonify({
            'success': False,
    ...
    try:
        ...
        return jsonify({
            **analytics,
            ...
            'hourly_stats': {},  # Empty hourly stats for now to avoid JS errors
            'learning_status': {
        ...
        maze_logger.log_error(e, {'endpoint': 'get_analytics'})
...
# Apply monitoring
health_monitor = setup_monitoring(app, db_session_manager)
...
def get_database_stats():
    ...
    try:
        ...
        maze_logger.log_error(e, {'endpoint': 'get_database_stats'})
...
def cleanup_database():
    ...
    try:
        ...
        maze_logger.log_captcha_event('database_cleanup', {
            'deleted_entries': deleted
        }, getattr(g, 'client_ip', None))
        return jsonify({'deleted_entries': deleted})
    except Exception as e:
        maze_logger.log_error(e, {'endpoint': 'cleanup_database'})
        return jsonify({'error': 'Cleanup failed'}), 500

if __name__ == '__main__':
    print("🚀 Starting Production-Ready Maze Captcha System with Complete Monitoring")
    ...
    print(f"🏥 Health check: http://localhost:8080/admin/health")
    print(f"📈 Metrics: http://localhost:8080/admin/metrics")
    ...
    try:
        ...
        print(f"⚠️  Warning: Could not clean up database on startup: {e}")
